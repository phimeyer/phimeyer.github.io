...
Dieses Tutorial ist zum großen Teil eine Wiederholung von den bisherigen Tutorials und zu einem kleinen Teil ein Vorgriff auf noch kommende Tutorials. 

In den folgenden Abschnitten werden wir alle nötigen Schritte einer Textanalyse durchgehen. Dafür verwenden wir natürlich das `quanteda`-Paket. In den vorherigen Tutorials wurden euch bereits einige der Techniken aus diesem Tutorial vorgestellt. Das Ziel dieses Tutorials ist es, euch ein sicheres Verständnis zu vermitteln, welche Schritte bei einer Textanalyse tatsächlich gemacht werden müssen. Außerdem dienen die gewollten Wiederholen dazu, dass ihr sicherer und geübter im Umgang mit `quanteda` und der Textanalyse werdet.

# 2. `quanteda`

Das [quanteda-Paket](https://quanteda.io/) ist ein Textanalyse-Paket für `R`. Es deckt nahezu alles ab, was nötig ist, um eine Textanalyse durchzuführen. Natürlich bietet `quanteda` auch eine klare und umfangreiche Dokumentation und eine Vielzahl an Tutorials (welche wir im Laufe des Seminars auch nutzen werden). 
...
Die Pflichttext für diese Sitzung von [Welbers, van Atteveldt und Benoit, 2017](https://www.tandfonline.com/doi/abs/10.1080/19312458.2017.1387238) bietet eine detailliertere Erklärung aller Schritte, welche wir jetzt besprechen werden.  
...
Im folgenden werden wir die klassischen ersten Schritte für eine Textanalyse durchsprechen: 
...
### 3.1 Schritt 1: Text importieren und einen Quanteda-Korpus erstellen

Der erste Schritt einer Textanalyse ist immer die zu analysierenden Texte in `R` einzulesen. Textdateien werden in einer Vielzahl von Formaten gespeichert: einfache txt-Files, csv-Dateien, html-Dateien und natürlich PDF. Erfahrungsgemäß machen einfache txt-Dateien die wenigsten Probleme. Eine Anleitung wie ihr txt-Dateien einlest findet ihr dem zweiten Tutorial zu dieser Sitzung (Titel: "Erste Schritte mit `quanteda`"). Das Einlesen von Textdaten mit `quanteda` funktioniert am effizientesten mit dem `readtext`-Paket und dem `readr`-Paket. 
...
```{r eval=T}
sotu_corpus <-  corpus(sotu_data, text_field = "text")  ## create the corpus
sotu_corpus
```
...
Wir können Texte natürlich auch von den lokalen Festplatten lesen (das ist in den meisten Fällen auch der "normale" Weg, da wir ja zu erst die Dokumente herunterladen). Hierfür müssen wir lediglich den Pfad angeben:
...
Die meisten Textanalyseverfahren basieren nur auf den Häufigkeiten von Wörtern in Dokumenten. Dieses Vorgehen wird als **Bag-of-Words**-Annahme bezeichnet: die Texte sind die Säcke in denen einzelne Wörter stecken. Interpunktion, Wortreihenfolge, Füllwörter und einzelne häufig vorkommende Wörter werden in diesem Vorgehen bereits in den ersten Schritten gelöscht. Obwohl dadurch viele relevante Informationen über die Reihenfolge der Wörter und die Syntax ignoriert werden, hat sich dieser Ansatz als sehr leistungsfähig und effizient erwiesen.
...
In diesem Matrixformat können wir jetzt Analysen mit unseren Texten durchführen, wie z. B. die Analyse verschiedener Frames in Bezug auf Katzen, oder die Berechnung der Ähnlichkeit zwischen dem dritten Satz und den ersten beiden Sätzen.

#### 3.2.1 Das sogenannte pre-processing: die DFM aufarbeiten und bereinigen

Die direkte Umwandlung eines Textes in einen DFM ist sehr vereinfachend. Beachtet z. B., dass die Wörter "Katzen" und "Katze" unterschiedliche Spalten erhalten. Außerdem sind "Hannover" und "Lehre" genauso unterschiedlich wie "Katzen" und "Katze". Das ist ein Problem, vor allem da wir eher an der Tatsache interessiert sind, dass es in den Sätzen um Katzen geht und weniger um das spezifische Wort an sich. Wir wollen also den Inhalt analysieren. Weiteren sind Textanalysen vor allem dann aussagekräftig, wenn weniger interessante Wörter wie "die" oder selte Wörter wie "Unviersität" ignoriert werden. 
...
Zusätzlich erstellen wir eine zweite DFM mit unseren State-of-the-Union-Reden (siehe oben) und führen dort zusätzlich ein Stemming durch (stemming fünktioniert mit der deutschen Sprache weniger gut, weshalb unsere Beispielsätze hierfür nicht geeignet sind). Einfach ausgedrückt, werden beim Stemming Wörter auf ihren Wortstamm reduziert. Dadurch werden einige Teile an den Wortenden entfernt und damit verschiedene Formen desselben Wortes ignoriert. Zum Beispiel Singular versus Plural ("gun" oder "gun-s") und verschiedene Verbformen ("walk", "walk-ing", "walk-s"). 
...
Es gibt weitere Techniken zum pre-processing. Für weitere Details lest bitte den Text von [Welbers, van Atteveldt und Benoit, 2017](https://www.tandfonline.com/doi/abs/10.1080/19312458.2017.1387238).
...
Wir können die Funktion `dfm_trim` verwenden, um bestimmte Wörter zu entfernen. Im Beispiel definieren wir, dass wir alle Wörter entfernen wollen, für die die Häufigkeit (d. h. der Summenwert der Spalte im DFM) unter 10 liegt.
...
Sie können auch nur Teile von unserem Korpus untersuchen. Zum Beispiel, indem man nur die Obama-Reden betrachtet. Um eine DFM zu subsetten/zu zerteilen, nutzen wir die `dtm_subset`-Funktion.
...
Die `kwic`Funktion kann auch verwendet werden, um eine Analyse auf einen bestimmten Suchbegriff zu fokussieren. Wir können die Ausgabe der Funktion kwic verwenden, um eine neue DFM zu erstellen, bei dem nur die Wörter innerhalb des angezeigten Fensters in die Matrix aufgenommen werden. Mit dem folgenden Code wird ein DFM erstellt, die nur die Wörter enthält, die innerhalb von 10 Wörtern vor oder nach dem Begriff terror* (terrorism, terrorist, terror, etc.) vorkommen.
...
Wie würder diese Kette von Befehlen in einer Pipeline aussehen? Probiert es mal aus!
...
#### 3.3.5 Erstellen von guten Wörterbüchern
