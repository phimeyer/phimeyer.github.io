...
Neben der klassischen Wordcloud lassen sich mit `quanteda` natürlich noch genauere Einblicke in unsere Texte realisieren. Als erstes ist die die KWIC-Methde (Key-Word-In-Context) zu nennen. Hierbei werden der Satzkontext von von euch definierter Suchbegriffe extrahiert und dargestellt. 
...
Neben den Häufigkeiten eines Wortes können wir auch die Position des Wortes in dem jeweiligen Text untersuchen. Das `quanteda`-Paket sellt die Funktion `textplot_xray` zur Verfügung: 
...
Die x-Achse repräsentiert die Position des jeweiligen Wortes innhalb des Textes. Wie wir sehen hat George W. Bush im Jahr 2005 das Wort nicht nur am Häufigsten von allen Präsidenten verwendet (siehe Plot zur Häufigkeit), sondern auch über seine gesamte Rede hinweg immer mal wieder verwendet. Dieses erste deskriptive Ergebnisse könnten wir jetzt im Kontext der 2005 vorherrschenden politischen Entwicklungen (Irakkrieg, großflächige Abhörungen von US-Bürgern etc.) analysieren. 

### 3.2 Worthäufigkeiten 

Mittels KWIC können wir, sehr umständlich, Worthäufigkeiten für spezielle Wörter untersuchen. Zum Glück bietet `quanteda` mit der `textstat_frequency`-Funktion einen deutlich einfachere Weg. Anstelle einer umständlichen eigenen Berechnung der verschiedenen Metriken, gibt uns `textstat_frequency` die Metadaten, die Anzahl der Dokumente, in denen das Wort vorkommt und vieles mehr aus.
