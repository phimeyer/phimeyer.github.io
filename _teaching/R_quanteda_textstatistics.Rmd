...
In diesem Tutorial wird es um generelle Textmetriken und Textvergleiche gehen. Da [Cornelius Puschmann](http://inhaltsanalyse-mit-r.de/metriken.html) zu diesem Thema bereits eine sehr gute Übersicht geliefert hat, werde ich seine Erklärungen, seinen Code und seine Darstellungen zu großen Teilen übernehmen und mit eigenen Daten wiederholen. 

Umgangssprachlich beschrieben, werdet ihr in diesem Tutorial lernen eure Texte kennenzulernen. Dafür werden wir uns Worthäufigkeiten und auch die Ähnlichkeit von Texten anschauen. Diese grundlegenden Statistiken sind relevant für die weiterführenden Methoden, die wir im Laufe des Seminars noch näher kennenlernen, und sie helfen uns bereits bei der Beantwortung unserer Fragestellungen. So habe ich in [meiner eigenen Arbeit](https://doi.org/10.1080/10584609.2020.1784329) zum Beispiel die Textähnlichkeit von Pressemitteilungen des Bundesverfassungsgericht und Zeitungsartikeln als abhängige Variable verwendet, um die Berichterstattung über Gerichtsentscheidungen zu analysieren.
...
Hierfür werden wir die Antrittsreden der US Präsidenten aus dem `quanteda`-Paket verwenden (für eine Liste aller in `quanteda` gespeicherten Korpora: [quanteda.corpora](https://github.com/quanteda/quanteda.corpora)). 
...
dtm_speeches <-  inaug_speeches %>% dfm(remove_numbers = TRUE, remove_punct = TRUE, remove_symbols = TRUE, remove = stopwords("english")) # wir entfernen alle Füllwörter, die Interpunktion, Symbole, Zahlen
dtm_speeches
```
...
Neben der klassischen Wordcloud lassen sich mit `quanteda` auch relativ einfach weitere noch genauere Einblicke in unsere Texte realisieren. Als erstes ist die die KWIC-Methde (Key-Word-In-Context) zu nennen. Hierbei werden der Satzkontext von von euch definierter Suchbegriffe extrahiert und dargestellt. 

Mit `quanteda` ist es möglich den Kontext einzeler Wörter oder auch auch ganze Sätze darzustellen. 

```{r}
kwic(inaug_speeches, "war", window = 2) # der window befehl definiert die anzahl der kontextwörter die angezeigt werden
```

Beachtet bitte das der KWIC-Befehl auf den Korpus und nicht auf die zuvor erstellte DTM angewendet wurde.

Mittels dieser einfachen Methode lassen sich jetzt weitere Analyseschritte realisieren. So können wir zum Beispiel die Häufigkeit und Verteilung von Tokens/Sequenzen berechnen, die die Begriffe "freedom" und "war".
...
term2 <- kwic(inaug_speeches, "war") %>%  # KWIC mit dem Wort "freedom"
  as.data.frame() %>%  # Transformation in einen Datensatz
  group_by(docname, keyword) %>% # Gruppierung
  mutate(count = n()) %>% # Berechnung wie oft das Wort "Freedom" vorkommt
...
Neben den Häufigkeiten eines Wortes können wir auch die Position des Wortes in dem jeweiligen Text untersuchen. Das `quanteda`-Paket sellt hiervor die Funktion `textplot_xray` zur Verfügung: 
...
Die x-Achse repräsentiert die Position des jeweiligen Wortes innhalb des Textes. Wie wir sehen hat Georg W. Bush im Jahr 2005 das Wort nicht nur am Häufigsten von allen Präsidenten verwendet (siehe Plot zur Häufigkeit), sondern auch über seine gesamte Rede hinweg immer mal wieder verwendet. Dieses erste deskriptive Ergebniss könnten wir jetzt im Kontext der 2005 vorherrschenden politischen Entwicklungen (Irakkrieg, großflächige Abhörungen von US-Bürgern etc.) analysieren. 

### 3.2 Worthäufigkeiten 

Mittels KWIC können wir, sehr umständlich, Worthäufigkeiten für spezielle Wörter zu untersuchen. Zum Glück bietet `quanteda` mit der `textstat_frequency`-Funktion einen deutlich einfachere Weg. Anstelle einer umständlichen eigenen Berechnung der verschiedenen Metriken, gibt uns `textstat_frequency` die Metadaten, die Anzahl der Dokumente, in denen das Wort vorkommt und vieles mehr aus.
...
Da unser Antrittsredenkropus die Parteien der Präsidenten enthält, können wir mit `textstat_frequency` die Häufigkeiten nach Parteien gruppieren und mittels `arrange` ordnen:
...
Die häufigsten Begriffe können wir natürlich auch pro Partei einzeln visualisieren. Der Einfachheit halber werden wir nur die Republikaner und die Demokratien plotten.
...
Natürlich haben wir im Vorhinein nicht sehr viel Arbeit in das Identifizieren von weiteren Stoppwörtern und andere hochfrequente aber eher bedeutungslose Wörter investiert. Wäre das der Fall gewesen, dann hätten wir hier sicherlich noch deutlich aussagekräftigere Ergebnisse gefunden.
...
Der Lambda- und z-Wert (z-standardisiertes Lambda) geben die Assoziationsstärke der Begriffspaare an. Je höher der Lambdawert, desto höher ist die Wahrscheinlichkeit das die zwei Begriffe auf einander folgen. 
...
Die Berechnung der Wortähnlichkeiten wird durch die `textstat_simil`-Funktion realisiert. Nutzt bitte die help-Funktion (`?textstat_simil` um die weiteren Methoden zur Berechnung vo Wort-/Textähnlichkeit kennenzulernen). Als Input für die Funktion erstellen wir eine DFM, in der jeder Satz einer Beobachtung (bzw. "einem Dokument") entspricht. Für unser Beispiel berechnen wir die Wortähnlichkeit zu den Begriffen "freedom" und "war".
...
Was sehen wir hier? Das Wort mit der höchsten Ähnlichkeit zur "Freedom" ist "bulwarks" also Bollwerke (womöglich interpretierbar mit: "Amerika als Bollwerk des Friedens in der Welt") und das Wort mit der höchstens Ähnlichkeit zu "War" ist "retained" also einbehalten oder Rücklage. Letzteres macht weniger Sinn, als zum Beispiel das Wort mit zweithöchsten Ähnlichkeit "calamaties" also Katastrophen. Man muss jedoch sagen, dass wir in Punkt 3.1 dieses Tutorials gesehen haben, dass beide Wörter nicht in jeder Rede ähnlich oft vorkommen. 

Wie dem auch sei, auch hier zeigt sich, dass wir zumindest etwas mehr Arbeit in die Identifizierung und Entfernung weiterer eher unbrauchbarer Wörter hätten stecken müssen. Weiterhin sieht man hier auch, dass Ähnlichkeitsmaße immer eine solide und große Datenbasis brauchen und das das Zielwort möglichst häufig (gleichverteilt) vorkommen sollte. 
...
Der Begriff "Government" ist nach diesem Eregbnis am weitestens von dem Begriff "freedom" entfernt, gefolgt von "people" and "US". Da wir aber von Kapitel 3.2 wissen, dass diese Begriffe auch die häufigsten Begriffe in unserem Korpus (und vermutlich auch über alle Reden gleich verteilt häufig auftreten) sind, ist dieses Ergebnis nicht verwunderlich. In einer Forschungsarbeit müssten wir diese Eregbnisse nutzen und unsere Preprocessing-Schritte verbessern und um damit unsere Eregbnisse zu verbessen. 
...
Im folgenden Beispiel inspizieren wir die Ähnlichkeit der 2005er Antrittsrede von Georg W. Bush mit allen anderen Antrittsreden. Dafür erstellen wir in einem ersten Schritt einen Datensatz mit den Ähnlichkeitsmaßen und im zweiten Schritt visualisieren wir die Maße: 
...
Bei diesen beiden Beispielen wird schnell klar, dass sich Trump vor allem an seinem Vorgänger Präsident Obama abgearbeitet hat und das der Name auch sehr zentral in dieser Rede ist. Während für die Rede von Obama die Begriffe "creed" und "journey" sehr zentral sind. In einer Analyse z.B. wie unterschiedlich die US Präsidenten ihre Antrittsreden framen, wäre ein solches erstes deskriptive Einsicht sicherlich sehr hilfreich. Die graue Referenzkategorie zeigt Begriffe an, welche in den anderen Reden auftreten, aber nicht in der von uns untersuchten Rede. 

# 5. Lesbarkeit/Komplexität von Texten

### 5.1 Diversität von Texten

Wie lässt sich die Diversität von Texten messen und vergleichen? Haben einige Präsidenten eine geringere Vielfalt an Wörtern genutzt als andere? Stimmt die Annahme, das Donald Trump eine einfacherer Sprache nutzt als seine Amtsvorgänger?
...
Wir sehen das vor allem die Reden der ersten Präsidenten eine hohe lexikalische Vielfalt aufgewiesen haben. Gerade die beiden Reden von Georg Washington 1789 und 1793 und die Reden von Madison 1809 und 1813 stechen hevor. Wichtig hierbei ist zu wissen, dass die Verwendung von Eigennamen, von Jargon und auch die Textlänge einen Einfluss auf die Textdiversität haben. Umso erstaunlicher ist der Befund für die Reden von Washington, ist doch seine 1789er die kürzteste Rede im gesamten Korpus mit 147 Wörtern und vier Sätzen. 
...
Ähnlich der Diversitätsmetriken gibt uns die Funktion wieder eine Vielzahl an unterschiedlichen Messmethoden aus. Schauen wir uns das etwas genauer an und analysieren nur die Antrittsreden von Carter bis Trump. Der Einfachhalb halber nutzen wir auch hier nur Messmethode: 
...
Wir sehen hier, dass die Reden von Carter und Obama 2013 im Vergleich zu den Reden von Bush senior und Donald Trump eher schwerer verständlich waren. Dieses Bild verhärtet sich, wenn wir uns die Reden aller Präsidenten, visualisiert nach Parteien anschauen: 
